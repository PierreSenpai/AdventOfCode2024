...
public class Day10Part2 {
    ...
    public void processInput() {
        ...
        for (int[] start : pathStarts) {
            sum += calculateRating(start[0], start[1]);
        }
    ...
    public int calculateRating(int x, int y) {
        return calculateRating(x, y, 0);
    }

    public int calculateRating(int x, int y, int value) {
        if (value == 9)
            return 1;
        ...
        if (x + 1 < map.length && map[x + 1][y] == value + 1)
            sum += calculateRating(x + 1, y, value + 1);

        if (y + 1 < map[x].length && map[x][y + 1] == value + 1)
            sum += calculateRating(x, y + 1, value + 1);
    ...
    public static void main(String[] args) {
        Day10Part2 d10p2 = new Day10Part2();
        d10p2.processInput();
    }
}
